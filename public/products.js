...
function consultarCategoria(id) {
    var urlCompleta = '/OperacionCategorias/ObtenerCategoria';
    ...
    request.done(function (response) {
        $(".subtitle").text(response.CategoriaName);
        $(".title").text("Productos de " + response.CategoriaName);

        consultarItemsDeCategoria(response);
    })
    request.fail(function (jqXHR, textStatus) {})
}

function consultarItemsDeCategoria(categoria) {
    var urlCompleta = '/OperacionCategorias/ObtenerItemsCategoria';
    ...
    request.done(function (response) {
        agregarItemDeCategoria(response);
    })
    request.fail(function (jqXHR, textStatus) {})
}
...
function elementosDelItem(element) {
    var elementos = "" +
        "<a href='#'>" +
        "<img src='" + element.MainImageURL + "' alt='product img' class='card-img products-img'>" +
        "<div class='card-body d-flex flex-column mt-auto'><h4 class='card-title mb-0'>" + element.Title + "</h4><p class='card-text'>" + element.ShortDescription + "</p></div></a>";
    if (sessionStorage.getItem('usuario') != null && sessionStorage.getItem('usuario') != "null") {
        elementos = elementos +
            "<div class='product-edit'>" +
            "<a href='#' onclick='itemSeleccionadoParaModificar(" + element.ProyectId + ")' data-toggle='modal' data-target='#editProduct' class='btn btn-sm btn-block btn-success editar-prod'>" +
            "<i class='fa fa-pen fa-lg'></i>" +
            "</a>" +
            "<a href='#' onclick='itemSeleccionado(" + element.ProyectId + ")' data-toggle='modal' data-target='#deleteProduct' class='btn btn-sm btn-block btn-danger eliminar-prod'>" +
...
function guardarItemNuevo() {
    ...
    var imagenSecundaria = document.getElementById("imagen-secundaria-item").files;
    ...
    AgregarItemNuevo(url, nombreProducto, productDescription, resumenProducto, specs1, specs2, specs3, specs4, specs5, form)
}

function AgregarItemNuevo(url, nombreProducto, productDescription, resumenProducto, specs1, specs2, specs3, specs4, specs5, form) {
...
function guardarModificaciones() {
    ...
    var productDescription = document.getElementById("productDescription-Edit").value;
    var resumenProducto = document.getElementById("resumenProducto-Edit").value;

    AgregarModificaciones(nombreProducto, productDescription, resumenProducto, specs1, specs2, specs3, specs4, specs5);
}

function AgregarModificaciones(nombreProducto, productDescription, resumenProducto, specs1, specs2, specs3, specs4, specs5) {
